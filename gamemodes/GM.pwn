// #if defined CREDITS_LTRP_REBORN
...
// 			Developer	:	Alee Wonka
// 			Projects	:	Gamemode wonka
// 			Since		:	12-07-2024
// 			version		:	1.0 (BETA)
// 			Github 		:	https://github.com/Aleewonka
// 			Description	:	I Made this gamemode while its still at 
...
#include "DATA\Notification.inc"
#include "DATA/Streamer.inc"
...
#include "Data\INVEN.inc"
// #include "Event\PClick.inc"
...
#include "MAP\generalmap.inc"
#include "MAP\interiormap.inc"
#include "MAP\jobmap.inc"
#include "UI\UiMain.inc"
...
stock HidePanelS(playerid)
{
	for(new i = 0; i < 46; i++)
	{
		PlayerTextDrawHide(playerid, RP_NatunaTD[playerid][i]);
	}
	CancelSelectTextDraw(playerid);
}

stock ShowPanelS(playerid) //Radial Natuna
{
	if(playerid != INVALID_PLAYER_ID)
	{
		for(new i = 0; i < 46; i++)
		{
			PlayerTextDrawShow(playerid, RP_NatunaTD[playerid][i]);
		}
...
public OnPlayerConnect(playerid)
{
	...
	{
		StreamerMobileSettings(playerid);
		SendClientMessage(playerid, -1, "Selamat datang di version >> {FFFF00}Mobile!");
	} 
	else
	{
		...
		Streamer_SetRadiusMultiplier(STREAMER_TYPE_PICKUP, 1.5, playerid);
		SendClientMessage(playerid, -1, "Selamat datang di version >> {FFFF00}PC!");
	}
	SendClientMessage(playerid, -1, "{FFFF00}[IndolifeBot] -> {FFFFFF}Selamat Datang di Indolife Roleplay.");
	new PlayerIP[16], country[MAX_COUNTRY_LENGTH], city[MAX_CITY_LENGTH];
	g_MysqlRaceCheck[playerid]++;
	//mata
	Refreshoxtarget(playerid);
	// pemainic++;
	AntiBHOP[playerid] = 0;
	...
	RemoveMappingIndolife(playerid);
	CreatePlayerTextDraws(playerid);
	CreatePlayerTextDrawsId(playerid);
	...
	/*foreach(new ii : Player)
	{
		...
		{
			Info(playerid, "UCP mu terdeteksi sudah logifor(new n; n < 10; n++){
    TextDrawShowForPlayer(playerid, LogoTD[n]);}n!");
			KickEx(playerid);
		}
	}*/
	//inventory
	...
	for(new n; n < 10; n++){
    TextDrawShowForPlayer(playerid, LogoTD[n]);}
...
public OnPlayerSpawn(playerid)
{
	...
	// SetTimerEx("CircleUpdate", 3000, true, "d", playerid);
	return 1;
}
...
SetPlayerSpawn(playerid)
{
	if(IsPlayerConnected(playerid))
	{
		...
		else
		{
			...
			{
				// for(new i; i < 6; i++){
				// PlayerTextDrawShow(playerid, indlfHbe[playerid][i]);
				// }
				// circle[0] = CreatePlayerCircleProgress(playerid, 290.000000, 416.000000, COLOR_RED, COLOR_WHITE, 10.0, 0.5, 15.0);
				// circle[1] = CreatePlayerCircleProgress(playerid, 315.000000, 416.000000, COLOR_BLUE, COLOR_GREY, 10.0, 0.5, 15.0);
				// circle[2] = CreatePlayerCircleProgress(playerid, 340.000000, 416.000000, COLOR_ORANGE2, COLOR_GREY, 10.0, 0.5, 15.0);
				// for(new i = 0; i < 5; i++)
				// {
...
public OnPlayerEnterDynamicCP(playerid, checkpointid)
{
	...
	{
		ShowPlayerDialog(playerid, DIALOG_BUYPVCP, DIALOG_STYLE_LIST, "{7fffd4}Indolife Showroom", "Motorcycle\nMobil\nKendaraan Unik\nKendaraan Job", "Select", "Cancel");
	}
...
ptask PlayerUpdate[999](playerid)
{
	...
	if(pData[playerid][pInjured] == 1 && pData[playerid][pHospital] != 1)
    {
		...
		if(GetPVarInt(playerid,"GiveUptime"))
        {
            ...
            {
            	new str[64];
	            format(str, sizeof(str), "~r~%d~W~ detik.", (GetPVarInt(playerid, "GiveUptime") + 600) - gettime());
				PlayerTextDrawSetString(playerid, AsteraDeath[playerid][4], str);
			}
        }
...
// 		case 5:
// 		{
// 			ChannelId = DCC_FindChannelById("1222831049256599604");
// 			DCC_SendChannelMessage(ChannelId, message);
// 			return 1;
// 		}
// 		case 6://Korup
...
// 		case 8://Bot Admin
// 		{
// 			ChannelId = DCC_FindChannelById("1225584762664779887");
// 			DCC_SendChannelMessage(ChannelId, message);
// 			return 1;
// 		}
// 	}
...
function ShowDeath(playerid)
{
	PlayerTextDrawShow(playerid, AsteraDeath[playerid][0]);
	PlayerTextDrawShow(playerid, AsteraDeath[playerid][1]);
	PlayerTextDrawShow(playerid, AsteraDeath[playerid][2]);
	PlayerTextDrawShow(playerid, AsteraDeath[playerid][3]);
	PlayerTextDrawShow(playerid, AsteraDeath[playerid][4]);
}
function HideDeath(playerid)
{
	PlayerTextDrawHide(playerid, AsteraDeath[playerid][0]); 
	PlayerTextDrawHide(playerid, AsteraDeath[playerid][1]);
	PlayerTextDrawHide(playerid, AsteraDeath[playerid][2]);
	PlayerTextDrawHide(playerid, AsteraDeath[playerid][3]);
	PlayerTextDrawHide(playerid, AsteraDeath[playerid][4]);
}
