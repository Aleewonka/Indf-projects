...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_LOGIN)
    {
		...
		else
		{
			...
			else 
			{
				...
				ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Login to Indolife", lstring, "Login", "Quit");
			}
		}
        return 1;
    }
	if(dialogid == DIALOG_REGISTER)
    {
		...
		if (strlen(inputtext) <= 5) return ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Indolife", "Kata sandi minimal 5 Karakter!\nMohon isi Password dibawah ini:", "Register", "Tolak");
		
		if(!IsValidPassword(inputtext))
		{
			Error(playerid, "Sandi valid : A-Z, a-z, 0-9, _, [ ], ( )");
			ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Indolife", "Kata sandi yang anda gunakan mengandung karakter yang valid!\nMohon isi Password dibawah ini:", "Register", "Tolak");
			return 1;
		}
	...
	if(dialogid == DIALOG_AMOUNT)
	{
		if (response)
		{
			...
			if(sscanf(inputtext, "p</>d", jumlah)) {
				ShowPlayerDialog(playerid, DIALOG_AMOUNT, DIALOG_STYLE_INPUT, "Hesterial - {ffffff}Jumlah inv", "Kesalahan! Tidak sah\nMasukan jumlah item minimal 1 sampai 1jt", "Oke", "Batal");
			}
			else if(jumlah < 1 || jumlah > 100000) {
				ShowPlayerDialog(playerid, DIALOG_AMOUNT, DIALOG_STYLE_INPUT, "Hesterial - {ffffff}Jumalah inv", "Kesalahan! Tidak sah\nMasukan jumlah item minimal 1 sampai 1jt", "Oke", "Batal");
			}
			pData[playerid][pGiveAmount] = strval(inputtext);
			format(str, sizeof(str), "%s", inputtext);
			PlayerTextDrawSetString(playerid, GIVETD[playerid][2], str);
		}
	}
	...
	// 		new value = pData[playerid][pGiveAmount];
	// 		new itemid = pData[playerid][pSelectItem];
	// 		CallLocalFunction("OnPlayerDropItem", "d", playerid, itemid, InventoryData[playerid][itemid][invItem], value);
	// 	}
	// }
			...
			else if(bData[bizid][bType] == 3)
			{
				switch(listitem)
				{
					...
					case 1:
					{
						...
						ShowPlayerDialog(playerid, DIALOG_TOYBUY, DIALOG_STYLE_LIST, ""RED_E"Indolife RP: "WHITE_E"Player Toys", string, "Select", "Cancel");
					}
					case 2:
					{
						if(price == 0)
		...
		if(response) 
		{
			...
			foreach(new i : PVehicles)
			{
				if(carid == pvData[i][cVeh])
				{
					...
					format(pvData[i][cPlate], 32, "Indolife-%d", rand);
		...
		if(response)
		{
			switch(listitem)
			{
				...
				case 2: // change bone
				{
					...
					ShowPlayerDialog(playerid, DIALOG_TOYPOSISI, DIALOG_STYLE_LIST, ""RED_E"Indolife RP: "WHITE_E"Player Toys", finstring, "Select", "Cancel");
				}
				case 3:
				{
	...
	if(dialogid == DIALOG_BANKDEPOSIT)
	{
		...
		else
		{
			...
			ShowPlayerDialog(playerid, DIALOG_UNUSED, DIALOG_STYLE_MSGBOX, ""ORANGE_E"Indolife RP: "LB_E"Bank", lstr, "Close", "");
		}
	}
	...
	if(dialogid == DIALOG_VERIFYCODE)
	{
		if(response)
		{
			...
			if(strval(inputtext) == pData[playerid][pVerifyCode])
			{
				...
				ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration Indolife", lstring, "Register", "Quit");	
				return 1;
			}
	...
	if(dialogid == DIALOG_SERVERMONEY_STORAGE)
	{
		...
		else 
		{
			...
			ShowPlayerDialog(playerid, DIALOG_SERVERMONEY, DIALOG_STYLE_MSGBOX, "Indolife City Money", lstr, "Manage", "Close");
		}
	}
